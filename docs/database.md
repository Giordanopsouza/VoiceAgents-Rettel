...
`appointment_slot.status` is the flag availability and the dashboard read. `GET /api/availability?date=YYYY-MM-DD` returns only `available` rows for 2026-09-14 or 2026-09-15, using the slot's stable ID and ISO-8601 UTC timestamps. `POST /retell/check_availability` is the signed Retell adapter: it unwraps `args.date` from the documented custom-function payload, records the attempt, and returns those same slot IDs with speakable date and clock labels (`14:30` is "2:30 PM"). `POST /retell/book_appointment` is the signed booking adapter: it requires explicit confirmation args, derives a display-safe idempotency key from `call_id`, slot, dentist, and normalized patient name, then records the attempt through Failure Lab. The partial unique index is the booking lock. Both change in the same transaction. `GET /api/events` returns the current demo session's `request_event` rows in `created_at`, `id` order, limited to sanitized public fields (`call_id`, `idempotency_key`, attempt, type, outcome, and appointment ID). Dashboard availability polling does not write timeline rows; Retell tool wrappers record availability and booking attempts. `GET /api/failure-lab` and `PUT /api/failure-lab` toggle the process-local lost-response injector. When it is enabled, the first successful create for an idempotency key records `created_then_response_delayed` and then waits `FAILURE_LAB_DELAY_SECONDS` after commit; a retry records `duplicate_returned` without waiting.
